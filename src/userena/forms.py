...
class SignupForm(forms.Form):
    ...
    email = forms.EmailField(widget=forms.TextInput(attrs=dict(attrs_dict,
                                                               maxlength=75)),
                             label=_("Email") , 
                             help_text= 'البريد الإلكترونى الذى سجلت به فى الفيس بوك' , 
                                 )
...
from users.models import City , Team , WorkFeild

class mSignupForm(SignupFormOnlyEmail):
    GENDER_CHOICES = (
        (0, 'Male'),
        (1, 'Female'),
    )
    first_name = forms.CharField( label=_(u'First name'),max_length=15,required=True)
    middle_name = forms.CharField(label=_(u'Middle name'),max_length=15,required=True)
    last_name =forms.CharField(label=_(u'Last name'),max_length=15,required=True)
    mobile = forms.CharField(label=_(u'mobile'),max_length=11,required=True)
    gender =  forms.ChoiceField(  choices = GENDER_CHOICES ,initial=0 ,    label=_("Gender"))
    city = forms.ModelChoiceField(City.objects.all()  , initial = 0 , 
              help_text = "المدينة التى تقيم بها"
    )   
    team = DynamicChoiceField(widget=forms.Select(attrs={'disabled':'false'}), choices=(('-1','Select Team'),) , 
    help_text = "االعشيرة التى تريد الإنضمام إليها" 
    )
    monthly_payment  = forms.FloatField( _("Monthly payment") , help_text = "القيمة التى يمكنك المساهمة بها شهرياً" , initial = 50)
    workFeild = forms.ModelChoiceField(WorkFeild.objects.all() , 
help_text = "المهنة التى تتخصص بهاً"   , initial = 0)   
    profession  = forms.CharField(label = _("Profession") ,
    help_text = "التخصص فى هذه المهنة" , 
    max_length= 255 )
    def clean_team(self):
        try:
            teamObj = Team.objects.get(id = self.cleaned_data['team'] , city =   self.cleaned_data["city"] )
        except:
            raise forms.ValidationError(_('Please choose valid choice'))
    ...
    def save(self):
        ...
        user = super(mSignupForm, self).save()
        ...
        user_profile.workFeild = self.cleaned_data['workFeild']
        ...
        user_profile.myteam = self.cleaned_data['team']
        user_profile.save()
        
        #Warning this will disable email confirmation
        user.is_active = True
        user.save()
        return user
...
class EditProfileForm(forms.ModelForm):
    ...
    class Meta:
        ...
        exclude = ['user' , "privacy", "interests"]
